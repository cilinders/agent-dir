...
then
  ...
  then
    ...
    printf "Message:\n" >> TEMP_commit.txt
    ./ollamaScript.sh -pvc conf/ollamaConfig_ollama3-1.conf model/conf/test.conf "Respond with the message for the git commit, no additional commentary" >> TEMP_commit.txt
  else
    printf "Config file not found.\n"
  fi
elif [[ $1 == "-fs" ]]
